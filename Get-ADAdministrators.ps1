...
Get-ADGroupMember -Identity "Domain Admins" | Select DisplayName, SamAccountName, DistinguishedName | Export-Csv $($GlobalSettings.Get_Item("OutputFolder") + "\ADAdmins " + $(get-date -f yyyy-MM-dd) + ".csv") -notype